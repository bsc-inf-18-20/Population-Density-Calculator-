...
/***************************************************************************
 population_density_analysis
                                 A QGIS plugin
 Automates the calculation process of population density
 Generated by Plugin Builder: http://g-sherman.github.io/Qgis-Plugin-Builder/
                              -------------------
        begin                : 2024-11-30
        git sha              : $Format:%H$
...
class population_density_analysis:
    ...
    def connect_to_database(self, host, port, database, user, password):
        """Connect to a PostgreSQL/PostGIS database."""
        uri = QgsDataSourceUri()
        uri.setConnection(host, str(port), database, user, password)

        # Example: Add a PostGIS layer to QGIS
        uri.setDataSource("public", "population_table", "geom")  # Replace with your schema, table, and geometry column
        layer = QgsVectorLayer(uri.uri(False), "Population Density", "postgres")
        if layer.isValid():
            QgsProject.instance().addMapLayer(layer)
            QMessageBox.information(self.iface.mainWindow(), "Success", "Connected to database and loaded layer!")
        else:
            QMessageBox.critical(self.iface.mainWindow(), "Error", "Failed to load PostGIS layer!")
    ...
    def run(self):
        ...
        if result:
            self.connect_to_database(
                ...
                database='population density calculator',     # Replace with your database name
